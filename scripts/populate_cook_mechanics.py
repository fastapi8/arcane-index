...
FISH_SIZE_PREFIX = re.compile(r"^(?:giant|small|massive|colossal)\s+", re.IGNORECASE)
    ...
    try:
        ...
        for row, multiplier in targets:
            ...
            candidates, candidate_range, pre_level_candidates = raw_energy_candidates(
                row["solo_energy"], multiplier, meal_level_multiplier
            )
            confirmed_raw = next(
            ...
            if recomputed_solo != row["solo_energy"]:
                raise AssertionError(
                    f"verification failed for {row['name']}: {recomputed_solo} != {row['solo_energy']}"
                )
            ...
            if current_raw is None:
                ...
                legacy_candidates, _ = integer_candidates(row["solo_energy"], multiplier)
